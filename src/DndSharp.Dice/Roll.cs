...
/// </summary>
[JsonConverter(typeof(JsonIParsableSerializer<Roll>))]
public class Roll : List<Die>, IParsable<Roll>, IRollable
{
    /// <summary>
    ...
    /// Only use this if you're aware of the effects and prefer fewer <see cref="Die"/> in the roll
    /// </remarks>
    public void Condense()
    {
        ...
        foreach (var combination in combinations)
        {
            ...
            var output = new Die(sides.Value, count, type.Value, modifier);
            Add(output);
        }
    }
    ...
    public Task<RollResult> Results(RollType type = RollType.Regular, INumberGenerator? generator = null, CancellationToken token = default)
    {
        var rolls = Task.WhenAll(this
            .Select(dice => dice.Roll(type, generator, token)));
        return rolls.ContinueWith(t => new RollResult(t.Result));
    }
    ...
    /// <param name="token">The cancellation token to use</param>
    /// <returns>The roll results</returns>
    public RollResult ResultsSync(RollType type = RollType.Regular, INumberGenerator? generator = null, CancellationToken token = default)
    {
        var roll = Results(type, generator, token);
        return roll.GetAwaiter().GetResult();
    }
    ...
    public static Roll operator +(Roll left, Die right)
    {
        return [.. left, right];
    }
    ...
    /// <returns>The roll</returns>
    public static Roll operator +(Die left, Roll right)
    {
        return [left, .. right];
    }
    ...
    public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, [MaybeNullWhen(false)] out Roll result)
    {
        ...
        var rolls = Die.ParseAll(s).ToArray();
        if (rolls.Length == 0)
        {
            result = null;
            return false;
