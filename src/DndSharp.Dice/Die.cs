    ...
    int modifier = 0) : IParsable<Die>, IRollable
{
    ...
    public static Task<int[]> Roll(int count, int sides, INumberGenerator generator, CancellationToken token = default)
    {
        return Task.WhenAll(
            Enumerable
            .Repeat(1, count)
            .Select(_ => generator.Generate(sides, token)));
    }
    ...
    public async Task<DiceResult> Roll(RollType type = RollType.Regular, INumberGenerator? generator = null, CancellationToken token = default)
    {
        var gen = generator ?? DefaultNumberGenerator;
        if (type == RollType.Regular)
        {
            var regularRoll = await Roll(Count, Sides, gen, token);
            var regularTotal = regularRoll.Sum() + Modifier;
            return new(this, regularTotal, regularRoll, 0, [], type);
        }

        var firstRoll = await Roll(Count, Sides, gen, token);
        var firstTotal = firstRoll.Sum() + Modifier;

        var secondRoll = await Roll(Count, Sides, gen, token);
    ...
    /// <param name="token">The cancellation token to use</param>
    /// <returns>The value that was rolled</returns>
    public DiceResult RoleSync(RollType type = RollType.Regular, INumberGenerator? generator = null, CancellationToken token = default)
    {
        var result = Roll(type, generator, token);
        return result.GetAwaiter().GetResult();
    }

    /// <summary>
    /// Returns the current die with modified properties
    /// </summary>
    /// <param name="sides">The number of sides on the dice</param>
    /// <param name="count">The number of dice to be rolled</param>
    /// <param name="type">The type of damage this dice is for</param>
    /// <param name="modifier">The modifier for the dice</param>
    /// <returns>The modified die</returns>
    public Die Change(DamageType? type = null, int? modifier = null, int? count = null, int? sides = null)
    {
        type ??= Type;
        modifier ??= Modifier;
        count ??= Count;
        sides ??= Sides;
        return new(sides.Value, count.Value, type.Value, modifier.Value);
    }
    #endregion
    ...
    /// <summary>
    /// Checks if two <see cref="Die"/> instances are equal based on their properties.
    /// </summary>
    ...
    /// <summary>
    /// Checks if two <see cref="Die"/> instances are not equal based on their properties.
    /// </summary>
    /// <param name="left">The item to check</param>
    /// <param name="right">The item to check against</param>
    ...
    /// <summary>
    /// Implicitly converts a <see cref="Die"/> instance to a string representation of the dice.
    /// </summary>
    /// <param name="dice">The dice to format</param>
    public static implicit operator string(Die dice)
    {
        return FormatDice(dice);
    }

    /// <summary>
    /// Implicitly converts a string representation of dice into a <see cref="Die"/> instance.
    /// </summary>
    /// <param name="input">The dice to parse</param>
    public static implicit operator Die(string input)
    {
        return Parse(input);
    }
    ...
    public static Roll operator +(Die left, Die right)
    {
        return [left, right];
    }
    ...
    public static Die operator +(Die dice, int modifier)
    {
        return new Die(dice.Sides, dice.Count, dice.Type, dice.Modifier + modifier);
    }
    #endregion
    ...
    public static string FormatDice(Die dice)
    {
    ...
    /// <summary>
    /// Attempts to parse the given string input into a <see cref="Die"/> instance.
    /// </summary>
    /// <param name="input">The input string</param>
    /// <returns>The dice value or null if the input is invalid</returns>
    internal static Die? ParseInternal(string? input)
    {
        ...
        return new Die(sidesValue, countValue, typeValue, modifierValue);
    }

    /// <inheritdoc />
    public static Die Parse(string input, IFormatProvider? provider = null)
    {
    ...
    /// <inheritdoc cref="TryParse(string?, IFormatProvider?, out Die)" />
    public static bool TryParse([NotNullWhen(true)] string? input, [MaybeNullWhen(false)] out Die result)
    {
        return TryParse(input, null, out result);
    }

    /// <inheritdoc />
    public static bool TryParse([NotNullWhen(true)] string? input, IFormatProvider? provider, [MaybeNullWhen(false)] out Die result)
    {
    ...
    public static IEnumerable<Die> ParseAll(string input)
    {
    ...
    public static DamageType ParseDamageType(string input)
    {
        ...
        if (input.StartsWith('t')) return DamageType.Thunder;
